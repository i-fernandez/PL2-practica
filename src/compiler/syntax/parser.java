...
/** CUP v0.11a beta 20060608 generated parser.
  * @version Mon May 13 20:13:26 CEST 2019
  */
  ...
  public final java_cup.runtime.Symbol CUP$parser$do_action(
    ...
    throws java.lang.Exception
    {
      ...
      switch (CUP$parser$act_num)
		...
		String name = id.getLexema().toLowerCase();	
		if (scopeManager.containsSymbol(name))  //Comprueba que existe el identificador
		{
			SymbolIF s = scopeManager.searchSymbol(name);
			if (s.getType().getName() == "ENTERO")  //Comprueba el tipo
			{
				if (s instanceof SymbolConstant) 	// Comprueba que sea una constante
				{
					SymbolConstant sC = (SymbolConstant) s;
					RESULT = new EntOId((Integer) sC.getValue());
				} else {
					semanticErrorManager.semanticFatalError("ERROR: El identificador '" + name + "' no es una constante");
				}
			} else {
				semanticErrorManager.semanticFatalError("ERROR: Tipo de identificador '" + name + "' incorrecto");
			}
		} else {
			semanticErrorManager.semanticFatalError("ERROR: Identificador '" + name + "' no definido");
		}
		...
		Integer v = Integer.valueOf(e.getLexema());
		RESULT = new EntOId(v);
		 ...
		 RESULT = new IntOBool((TypeSimple) scopeManager.searchType("BOOLEANO")); 
          ...
          case 105: // intOBool ::= INTEGER 
            {
              IntOBool RESULT =null;
		 RESULT = new IntOBool((TypeSimple) scopeManager.searchType("ENTERO")); 
              CUP$parser$result = parser.getSymbolFactory().newSymbol("intOBool",41, ((java_cup.runtime.Symbol)CUP$parser$stack.peek()), ((java_cup.runtime.Symbol)CUP$parser$stack.peek()), RESULT);
            }
          return CUP$parser$result;
		...
		if (exp.getType().getName().equals("ENTERO"))
		{
			IdArray ida = new IdArray();			
			ida.setTemp (exp.getTemp ());
			ida.setIntermediateCode (exp.getIntermediateCode ());
			RESULT = ida;
		} else {
			semanticErrorManager.semanticFatalError("ERROR: Expresion de acceso a array no valida.");
		} 
		...
		if (scopeManager.containsSymbol(name))
		{
			...
			if (t instanceof TypeFunction)
			{
				...
				if (tf.checkParameters(pf.getParams()))
				{
					Variables vb = new Variables (tf.getReturnType());
					// Codigo intermedio
					ScopeIF scope = scopeManager.getCurrentScope ();
					IntermediateCodeBuilder cb = new IntermediateCodeBuilder (scope);
					TemporalFactory tF = new TemporalFactory (scope);
					LabelFactory lF = new LabelFactory();
					LabelIF l1 = lF.create (name);	
					TemporalIF temp = tF.create ();	
					cb.addQuadruple ("DATA_SUB", l1); 	// Prepara el R.A. del subprograma
					cb.addQuadruples (pf.getIntermediateCode ());	
					cb.addQuadruple ("CALL", l1);	
					cb.addQuadruple ("UNDO_SUB", temp);	// Deshace el RA y guarda el valor de retorno 
					vb.setTemp (temp);
					vb.setIntermediateCode (cb.create ());
					RESULT = vb;
				} else {
					semanticErrorManager.semanticFatalError("ERROR: Parametros incorrectos en llamada a " + name);
				}
			} else {
				semanticErrorManager.semanticFatalError("ERROR: " + name + " no es una funcion.");
			}
		} else {
			semanticErrorManager.semanticFatalError("ERROR: Funcion " + name + " no declarada.");
		}
		...
		{
			SymbolIF s = scopeManager.searchSymbol(name);
			TypeIF t = scopeManager.searchType(s.getType().getName());
			if (t instanceof TypeArray)			// Comprueba que es un array
			{
				TypeArray ta = (TypeArray) t;
				int elemSize = ta.getTypeBase ().getSize ();
				Variables v = new Variables(ta.getTypeBase());
				Variable var = new Variable (name, s.getScope ());
				// Codigo intermedio
				ScopeIF scope = scopeManager.getCurrentScope();
				TemporalFactory tF = new TemporalFactory (scope);
				IntermediateCodeBuilder cb = new IntermediateCodeBuilder (scope);
				Value minIndex = new Value (ta.getMinIndex());
				TemporalIF iTemp = ia.getTemp ();			// Temporal del idArray
				TemporalIF tempRel = tF.create ();			// Temporal para la posicion relativa
				TemporalIF tempVar = tF.create ();			// Temporal para la direccion de inicio
				cb.addQuadruples (ia.getIntermediateCode ());	
				cb.addQuadruple ("SUB", tempRel, ia.getTemp (), minIndex);	// Posiciones de desplazamiento
				cb.addQuadruple ("MUL", tempRel, tempRel, elemSize);		// Direcciones de desplazamiento
				cb.addQuadruple ("MVA", tempVar, var);						// Direccion de inicio
				cb.addQuadruple ("ADD", tempVar, tempRel, tempVar);			// Calcula la posicion del elemento
				v.setTemp(tempVar);
				v.setIntermediateCode (cb.create ());
				RESULT = v;
			} else {
				semanticErrorManager.semanticFatalError("ERROR: " + name + " no es un array.");
			}
		} else {
			semanticErrorManager.semanticFatalError("ERROR: Variable " + name + " no declarada.");
		}
		...
		if (scopeManager.containsSymbol(name))
		{
			ScopeIF scope = scopeManager.getCurrentScope();
			SymbolIF s = scopeManager.searchSymbol(name);
			Variables v = new Variables(s.getType());
			v.setSymbol (s);
			v.setName (name);
			TemporalFactory tF = new TemporalFactory (scope);
			IntermediateCodeBuilder cb = new IntermediateCodeBuilder (scope);
			...
			{
				SymbolConstant sb = (SymbolConstant) s;
				Value val = new Value (sb.getValue());
			...
			{
				semanticErrorManager.semanticFatalError("Tipo no reconocido en sentencia de asignacion de " + name);
			}
			v.setTemp (temp);
			v.setIntermediateCode (cb.create ());
			RESULT = v;
		} else {
			semanticErrorManager.semanticFatalError("ERROR: Variable " + name + " no declarada en scope " + scopeManager.getCurrentScope().getName ());
		}
		...
		SWriteLn sw = new SWriteLn ();
		ScopeIF scope = scopeManager.getCurrentScope();	
		IntermediateCodeBuilder cb = new IntermediateCodeBuilder(scope);
		cb.addQuadruple ("WRTLN");
		...
		if (ex.getType().getName() == "ENTERO")
		{
			SWriteInt sw = new SWriteInt ();
			// Codigo intermedio
			ScopeIF scope = scopeManager.getCurrentScope();
			IntermediateCodeBuilder cb = new IntermediateCodeBuilder(scope);
			cb.addQuadruples (ex.getIntermediateCode ());
			cb.addQuadruple ("WRTINT", ex.getTemp ());
			sw.setIntermediateCode (cb.create ());
			RESULT = sw;
			
		} else {
			semanticErrorManager.semanticFatalError("ERROR: Expresion en WRITEINT de tipo incorrecto.");	
		}
		...
		String texto = s.getLexema ();
		SWriteString sw = new SWriteString ();
		String data = Data.getKey ();
		ScopeIF scope = scopeManager.getCurrentScope ();
		IntermediateCodeBuilder cb = new IntermediateCodeBuilder (scope);
		Data.setCadena (texto, data);
		cb.addQuadruple ("WRTSTR", data);
		sw.setIntermediateCode (cb.create ());
		RESULT = sw; 
		...
		Parametros par = new Parametros(ex.getType());
		// Codigo Intermedio
		ScopeIF scope = scopeManager.getCurrentScope ();
		IntermediateCodeBuilder cb = new IntermediateCodeBuilder (scope);
		cb.addQuadruples (ex.getIntermediateCode ());
		cb.addQuadruple ("PARAM", ex.getTemp ());
		par.setIntermediateCode (cb.create ());
		RESULT = par; 
		...
		Parametros par = new Parametros(p, ex.getType());
		// Codigo Intermedio
		ScopeIF scope = scopeManager.getCurrentScope ();
		IntermediateCodeBuilder cb = new IntermediateCodeBuilder (scope);
		...
		ParFuncion pf = new ParFuncion (p);
		pf.setIntermediateCode (p.getIntermediateCode ());
		RESULT = pf; 
		...
		if (scopeManager.containsSymbol(name))
		{
			...
			{
				TypeProcedure tp = (TypeProcedure) t;
				if (tp.checkParameters(pf.getParams()))
				{
					// Codigo intermedio
					SentProcedure sentp = new SentProcedure();
					ScopeIF scope = scopeManager.getCurrentScope ();
					IntermediateCodeBuilder cb = new IntermediateCodeBuilder (scope);
					LabelFactory lF = new LabelFactory();
					TemporalFactory tF = new TemporalFactory (scope);
					...
					cb.addQuadruples (pf.getIntermediateCode ());	
					cb.addQuadruple ("CALL", l1);
					cb.addQuadruple ("UNDO_SUB", temp);
					sentp.setIntermediateCode (cb.create ());
					RESULT = sentp;
				} else {
					semanticErrorManager.semanticFatalError("ERROR: Parametros incorrectos en llamada a " + name);
				}				
			} else {
				semanticErrorManager.semanticFatalError("ERROR: " + name + " no es un procedimiento.");
			}
		} else {
			semanticErrorManager.semanticFatalError("ERROR: Procedimiento " + name + " no declarado.");
		}
		...
		Sentencias se = (Sentencias)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
		...
		if (!sTable.containsSymbol (name))
		{
			...
			SymbolIF s = sTable.getSymbol (name);
			if (!s.getType().getName().equals("ENTERO"))
			{
				semanticErrorManager.semanticFatalError("ERROR: Indice " + name + " de tipo incorrecto.");
			} else if (e1.getType().getName() != "ENTERO" || e2.getType().getName() != "ENTERO")
			{
				semanticErrorManager.semanticFatalError("ERROR: Expresion de tipo erroneo en sentencia FOR");
			} else {
				// Codigo Intermedio 
				SentFor sf = new SentFor (se.getReturnType ());
				LabelFactoryIF lF = new LabelFactory ();
				TemporalFactory tF = new TemporalFactory (scope);
				IntermediateCodeBuilder cb = new IntermediateCodeBuilder (scope);
				LabelIF l1 = lF.create ();
				LabelIF l2 = lF.create ();
				TemporalIF e1Temp = e1.getTemp ();
				TemporalIF e2Temp = e2.getTemp ();
				TemporalIF tempEval = tF.create ();		// Para evaluacion temporal  id = e2
				// Asignacion id = e1 
				Variable var = new Variable(name, s.getScope());
				cb.addQuadruples(e1.getIntermediateCode ());
				cb.addQuadruple ("MV", var, e1Temp);
				// Bucle
				cb.addQuadruples(e2.getIntermediateCode ());
				cb.addQuadruple ("LABEL", l1);
				cb.addQuadruple ("GR", tempEval, var, e2Temp);		// Evalua si e2 > var
				cb.addQuadruple ("BRT", tempEval, l2);
				cb.addQuadruples (se.getIntermediateCode ());
				cb.addQuadruple ("INC", var);		// Incrementa var en 1
				cb.addQuadruple ("BR", l1);
				cb.addQuadruple ("LABEL", l2);
		...
		SentElse se = new SentElse (s.getReturnType()); 		
		...
		if (!ex.getType().getName().equals("BOOLEANO"))
		{
			semanticErrorManager.semanticFatalError("ERROR: Sentencia IF con tipo erroneo.");
		} else {
			SentIf si = new SentIf(s.getReturnType());
			if (si.setReturnType(se.getReturnType()) == false)
			{
				semanticErrorManager.semanticFatalError("ERROR: Sentencias RETURN incompatibles.");
			} else {
				// Codigo Intermedio
				ScopeIF scope = scopeManager.getCurrentScope ();
				TemporalFactory tF = new TemporalFactory (scope);
		...
		String tv = v.getType().getName();
		String te = e.getType().getName();
		if (tv.equals(te))
		{
			...
			TemporalFactory tF = new TemporalFactory(scope);
			IntermediateCodeBuilder cb = new IntermediateCodeBuilder(scope);			
			TemporalIF vTemp = v.getTemp ();
			TemporalIF eTemp = e.getTemp ();
			...
			semanticErrorManager.semanticFatalError("ERROR: expresiones incompatibles (" + tv + " - " + te + ")");
		}
		...
		SentReturn sr = new SentReturn (ex.getType ());
		// Codigo intermedio
		ScopeIF scope = scopeManager.getCurrentScope ();
		IntermediateCodeBuilder cb = new IntermediateCodeBuilder (scope);
		LabelFactory lF = new LabelFactory();
		...
		cb.addQuadruples (ex.getIntermediateCode ());
		cb.addQuadruple ("RET", ex.getTemp (), l1);
		...
		Sentencia se = new Sentencia ();
		se.setIntermediateCode (sp.getIntermediateCode ());
		RESULT = se; 
		...
		SWriteLn sw = (SWriteLn)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		 
		Sentencia se = new Sentencia (); 
		se.setIntermediateCode (sw.getIntermediateCode ());
		RESULT = se; 
		...
		int swleft = ((java_cup.runtime.Symbol)CUP$parser$stack.peek()).left;
		int swright = ((java_cup.runtime.Symbol)CUP$parser$stack.peek()).right;
		SWriteInt sw = (SWriteInt)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		...
		int swleft = ((java_cup.runtime.Symbol)CUP$parser$stack.peek()).left;
		int swright = ((java_cup.runtime.Symbol)CUP$parser$stack.peek()).right;
		SWriteString sw = (SWriteString)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		...
		Sentencia se = new Sentencia (sr.getReturnType ());
		se.setIntermediateCode (sr.getIntermediateCode ());
		RESULT = se; 
		...
		Sentencia se = new Sentencia (sf.getReturnType ());		
		se.setIntermediateCode (sf.getIntermediateCode ());
		RESULT = se; 
		...
		Sentencia se = new Sentencia (si.getReturnType ()); 		
		se.setIntermediateCode (si.getIntermediateCode ());
		RESULT = se; 
		...
		Sentencia se = new Sentencia (); 
		se.setIntermediateCode (sa.getIntermediateCode ());
		RESULT = se; 
		...
		ListSentencia ls = new ListSentencia (s.getReturnType ());
		ls.setIntermediateCode (s.getIntermediateCode ());
		RESULT = ls; 
		...
		ListSentencia l = new ListSentencia(s.getReturnType());
		if (l.setReturnType(ls.getReturnType()) == false)
		{
			semanticErrorManager.semanticFatalError("ERROR: Sentencias RETURN incompatibles.");	
		} else {
			// Codigo Intermedio
			...
			cb.addQuadruples (s.getIntermediateCode ());
			l.setIntermediateCode (cb.create ());
			RESULT = l;
		}
		...
		Sentencias s = new Sentencias(ls.getReturnType());
		s.setIntermediateCode (ls.getIntermediateCode ());
		RESULT = s;
		...
		ScopeIF scope = scopeManager.getCurrentScope();
		Boolean value = vb.getValue();
		ExprLogica e = new ExprLogica(scope);
		TemporalFactory tF = new TemporalFactory(scope);
		IntermediateCodeBuilder cb = new IntermediateCodeBuilder(scope);
		TemporalIF temp = tF.create();
		Value v = new Value(value);
		cb.addQuadruple ("MV", temp, v);
		e.setTemp (temp);
		...
		if (e1.getType().getName().equals("BOOLEANO"))
		{
			ScopeIF scope = scopeManager.getCurrentScope();
			ExprLogica e = new ExprLogica(scope);
			IntermediateCodeBuilder cb = new IntermediateCodeBuilder(scope);
			TemporalIF eTemp = e1.getTemp ();
			cb.addQuadruples (e1.getIntermediateCode ());
			cb.addQuadruple ("NOT", eTemp);
			e.setTemp (eTemp);
			e.setIntermediateCode (cb.create ());
			RESULT = e; 
		} else {
			semanticErrorManager.semanticFatalError("ERROR: expresiones incompatibles");
		}
		...
		TypeSimple t1 = (TypeSimple) e1.getType();
		TypeSimple t2 = (TypeSimple) e2.getType();
		if (t1.isCompatible(t2, "OR"))
		{
			ScopeIF scope = scopeManager.getCurrentScope ();
			ExprLogica e = new ExprLogica (scope);
			TemporalFactory tF = new TemporalFactory (scope);
			IntermediateCodeBuilder cb = new IntermediateCodeBuilder (scope);
			TemporalIF temp1 = e1.getTemp ();
			TemporalIF temp2 = e2.getTemp ();
			TemporalIF temp = tF.create ();
			cb.addQuadruples (e1.getIntermediateCode ());
			cb.addQuadruples (e2.getIntermediateCode ());
		...
		Integer value = Integer.valueOf(n.getLexema());
		ScopeIF scope = scopeManager.getCurrentScope();
		ExprArit e = new ExprArit(scope);
		TemporalFactory tF = new TemporalFactory(scope);
		IntermediateCodeBuilder cb = new IntermediateCodeBuilder(scope);
		TemporalIF temp = tF.create();
		Value v = new Value(value);
		...
		ScopeIF scope = scopeManager.getCurrentScope ();
		Expresion e = new Expresion(v.getType());
		TemporalFactory tF = new TemporalFactory (scope);
		IntermediateCodeBuilder cb = new IntermediateCodeBuilder (scope);
		TemporalIF temp = tF.create ();
		TemporalIF vTemp = v.getTemp ();
		cb.addQuadruples (v.getIntermediateCode ());
		int ic_size = v.getIntermediateCode ().size ();
		OperandIF op = v.getIntermediateCode ().get(ic_size - 1).getFirstOperand ();
		if (op instanceof Value) {
			e.setTemp (vTemp);
		} else  {
			// Saltar MVP si variable no global
			cb.addQuadruple ("MVP", temp, vTemp);
			e.setTemp (temp);
		}
		e.setIntermediateCode (cb.create ());		
		RESULT = e; 
		...
		Expresion e = new Expresion (exp.getType());		
		e.setTemp (exp.getTemp());
		e.setIntermediateCode (exp.getIntermediateCode ());
		RESULT = e; 
		...
		IntOBool io = (IntOBool)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		 RESULT = new TipoRetorno(io.getType()); 
		...
		CadIdVar ci = (CadIdVar)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-2)).value;
		int tleft = ((java_cup.runtime.Symbol)CUP$parser$stack.peek()).left;
		int tright = ((java_cup.runtime.Symbol)CUP$parser$stack.peek()).right;
		TipoVar t = (TipoVar)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		 RESULT = new ProcParam(ci.getIds(), t.getType()); 
              CUP$parser$result = parser.getSymbolFactory().newSymbol("procParam",21, ((java_cup.runtime.Symbol)CUP$parser$stack.elementAt(CUP$parser$top-2)), ((java_cup.runtime.Symbol)CUP$parser$stack.peek()), RESULT);
            }
          return CUP$parser$result;
		...
		ProcListParam pl = (ProcListParam)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
			RESULT = new ProcListParam(pl, pp); 
		...
		ProcListParam pl = (ProcListParam)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
		 RESULT = new ProcParenParam(pl); 
		...
		ScopeIF parentScope = scopeManager.getCurrentScope();
		SymbolTableIF parentSTable = parentScope.getSymbolTable();
		TypeTableIF parentTTable = parentScope.getTypeTable();
		String name = id.getLexema().toLowerCase();		
		ScopeIF currentScope = scopeManager.openScope(name);		
		SymbolTableIF currentSTable = currentScope.getSymbolTable();
		
		if (parentSTable.containsSymbol(name))
		{
			semanticErrorManager.semanticFatalError("ERROR: Subprograma " + name + " declarado repetido");
		} else {
			if (tr.getType() == null)	
			{
				// Procedimiento
				TypeProcedure tP = new TypeProcedure(parentScope, name);
				SymbolProcedure sub = new SymbolProcedure(parentScope, name, tP);
				parentTTable.addType(tP);
				parentSTable.addSymbol(sub);
				for (ProcParam item : pp.getParamList()) 
				{
					TypeIF tipo = item.getType();
					
					for (String prm : item.getParams())
					{
						if (currentSTable.containsSymbol(prm)) // Comprueba si existe en la tabla de simbolos actual
						{
							semanticErrorManager.semanticFatalError("ERROR: Parametro '" + prm + "' en procedimiento '" + name + "' repetido");
						} else {
							currentSTable.addSymbol(new SymbolParameter(currentScope, prm, tipo));
							tP.addParameterType(tipo);
				...
				// Funcion
				TypeFunction tF = new TypeFunction(parentScope, name);
				tF.setReturnType(tr.getType());
				SymbolFunction sub = new SymbolFunction(parentScope, name, tF);
				parentTTable.addType(tF);
				parentSTable.addSymbol(sub);
				for (ProcParam item : pp.getParamList()) 
				{
					TypeIF tipo = item.getType();
					for (String prm : item.getParams())
					{
						if (currentSTable.containsSymbol(prm)) // Comprueba si existe en la tabla de simbolos actual
						{
							semanticErrorManager.semanticFatalError("ERROR: Parametro '" + prm + "' en funcion '" + name + "' repetido");
						} else {
							currentSTable.addSymbol(new SymbolParameter(currentScope, prm, tipo));
							tF.addParameterType(tipo);
						}	
					}
				}
			}
		...
		RESULT = new CabProcedure(name, tr.getType());
		...
		Cuerpo cu = (Cuerpo)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
		int sleft = ((java_cup.runtime.Symbol)CUP$parser$stack.peek()).left;
		int sright = ((java_cup.runtime.Symbol)CUP$parser$stack.peek()).right;
		StmSubprogram s = (StmSubprogram)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		
		String name = cp.getId();
		if(!name.equals(cu.getEndId()))
		{
			semanticErrorManager.semanticFatalError("ERROR: Inicio y fin de subprograma '" + name + "' no coinciden.");
		} else if (!cp.getReturnTypeName().equals(cu.getReturnTypeName())) {
			semanticErrorManager.semanticFatalError("ERROR: Tipo de retorno de '" + name + "' no es valido.");
		} else {
			ScopeIF scope = scopeManager.getCurrentScope ();
			// Codigo intermedio			
			StmSubprogram ss = new StmSubprogram ();			
			IntermediateCodeBuilder cb = new IntermediateCodeBuilder (scope);
			LabelFactory lF = new LabelFactory();
			LabelIF l1 = lF.create(name);				
			cb.addQuadruple ("LABEL_SUB", l1); 				// Etiqueta del subprograma
			cb.addQuadruples (cu.getIntermediateCode ());
			cb.addQuadruples (s.getIntermediateCode ());
		...
		{
			RESULT = new TipoVar(scopeManager.searchType(name));
		} else {
			semanticErrorManager.semanticFatalError("ERROR: Tipo '" + name + "' no definido.");
		}
		...
		IntOBool ib = (IntOBool)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		 RESULT = new TipoVar(ib.getType()); 
              CUP$parser$result = parser.getSymbolFactory().newSymbol("tipoVar",16, ((java_cup.runtime.Symbol)CUP$parser$stack.peek()), ((java_cup.runtime.Symbol)CUP$parser$stack.peek()), RESULT);
            }
          return CUP$parser$result;
		...
		int idright = ((java_cup.runtime.Symbol)CUP$parser$stack.peek()).right;
		Token id = (Token)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		 RESULT = new CadIdVar(id.getLexema().toLowerCase()); 
		...
		CadIdVar c = (CadIdVar)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		 RESULT = new CadIdVar(c, id.getLexema().toLowerCase()); 
		...
		CadIdVar c = (CadIdVar)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-3)).value;
		int tleft = ((java_cup.runtime.Symbol)CUP$parser$stack.elementAt(CUP$parser$top-1)).left;
		int tright = ((java_cup.runtime.Symbol)CUP$parser$stack.elementAt(CUP$parser$top-1)).right;
		TipoVar t = (TipoVar)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
		
		ScopeIF scope = scopeManager.getCurrentScope();
		SymbolTableIF sTable = scope.getSymbolTable();
		ArrayList<String> ids = c.getIds();
		for (String str : ids) 			// Añade a la tabla de simbolos (si no existe)
		{
			if (sTable.containsSymbol(str))
			{
				semanticErrorManager.semanticFatalError("ERROR: Variable '" + str + "' repetida en el ámbito");
			} else {
				SymbolVariable sv = new SymbolVariable(scope, str, t.getType());
				sTable.addSymbol(sv);	
			}
		}
		...
		EntOId e1 = (EntOId)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-6)).value;
		int e2left = ((java_cup.runtime.Symbol)CUP$parser$stack.elementAt(CUP$parser$top-4)).left;
		int e2right = ((java_cup.runtime.Symbol)CUP$parser$stack.elementAt(CUP$parser$top-4)).right;
		EntOId e2 = (EntOId)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-4)).value;
		...
		IntOBool io = (IntOBool)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
		
		ScopeIF scope = scopeManager.getCurrentScope();
		TypeTableIF tTable = scope.getTypeTable();
		String name = id.getLexema().toLowerCase();
		if (scopeManager.containsType(name))			// Comprueba si ya existe
		{
			semanticErrorManager.semanticFatalError("ERROR: Tipo '" + name + "' repetido");
		} else {
			int min = e1.getValue();
			int max = e2.getValue();
			if (min > max)
			{
				semanticErrorManager.semanticFatalError("ERROR: Indices de '" + name + "' no validos");
			} else {
				TypeArray tipoArray = new TypeArray(scope, name, min, max, io.getType());
				tTable.addType(tipoArray);
			}
		}
		...
		RESULT = new ValorConst(v.getValue(), scopeManager.getCurrentScope());
		...
		int v = Integer.valueOf(e.getLexema());
		ScopeIF scope = scopeManager.getCurrentScope();
		RESULT = new ValorConst(v, scope);
		...
		ValorConst v = (ValorConst)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
		...
		if (sTable.containsSymbol(name))
		{
			semanticErrorManager.semanticFatalError("ERROR: Constante '" + name + "' repetida");
		} else {
			SymbolConstant sc = new SymbolConstant(scope, name, v.getType());
		...
		Cuerpo c = new Cuerpo(id.getLexema().toLowerCase(), s.getReturnType());
		ScopeIF scope = scopeManager.getCurrentScope ();
		...
		if (name.equals("main")) { 
			...
			// Numero de parametros del subprograma
			TypeProcedure tp = (TypeProcedure) scopeManager.searchType (name);
			int nParams = tp.getParametersCount ();
			LabelFactory lF = new LabelFactory();
			LabelIF l1 = lF.create(name);	
			cb.addQuadruple ("FIN_SUB", l1, nParams);	 	// Etiqueta fin de subprograma
		} 
		...
		String name = id.getLexema().toLowerCase();		
		ScopeIF scope = scopeManager.openScope("main");
		// Introduce los tipos basicos
		TypeTableIF tTable = scope.getTypeTable();
		tTable.addType(new TypeSimple(scope, "ENTERO"));
		tTable.addType(new TypeSimple(scope, "BOOLEANO"));
		RESULT = new CabModule(name);		
		...
		Cuerpo cu = (Cuerpo)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		
		if (cm.getName().equals(cu.getEndId()))
		{
			AxiomImpl ax = new AxiomImpl ();
			ax.setIntermediateCode (cu.getIntermediateCode ());
			RESULT = ax;
		} else {
			semanticErrorManager.semanticFatalError("ERROR: Inicio y fin de modulo '" + cm.getName() + "' no coinciden.");
		}
		...
		Axiom ax = (Axiom)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
  		...
  		IntermediateCodeBuilder cb = new IntermediateCodeBuilder (scopes.get (0));
  		cb.addQuadruple("DATA");
  		LabelFactory lF = new LabelFactory();
		LabelIF l1 = lF.create("main");
  		cb.addQuadruple ("LABEL", l1); 		// Etiqueta de inicio del programa
		cb.addQuadruples (ax.getIntermediateCode ());
		ax.setIntermediateCode (cb.create ());
		ax.printCode();
		finalCodeFactory.setEnvironment (CompilerContext.getExecutionEnvironment ());
		finalCodeFactory.create (ax.getIntermediateCode ());		
